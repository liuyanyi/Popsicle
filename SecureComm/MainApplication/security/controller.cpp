...
string controller::aes_encrypt(const string& plain_text, const string& key, const int mode)
{
	...
	//有修改
	Util::string_to_byte(key.substr(0, 16), k);
	//Util::hex_string_to_byte(key, k);

	byteu ivb[16];
	Util::string_to_byte(key.substr(16), ivb);
...
bool controller::file_handle(wstring input_file_path, wstring output_file_path, string key, int mode)
{
	FILE* file1;
	FILE* file2;

	_wfopen_s(&file1, input_file_path.c_str(), L"rb");
	_wfopen_s(&file2, output_file_path.c_str(), L"wb");
	...
	if (file1 != nullptr && file2 != nullptr)
	{
		...
		if (mode == FILE_ENCRYPT)
		{
			fseek(file1, -remainder, SEEK_END);
		...
		cout << u8"时间" << dur / CLOCKS_PER_SEC << "s" << endl;
