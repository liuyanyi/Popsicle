...
NativeWindowHelper::NativeWindowHelper(QWindow* window,
	...
	: QObject(window), d_ptr(new NativeWindowHelperPrivate())
{
	d_ptr->q_ptr = this;
...
bool NativeWindowHelper::nativeEventFilter(void* msg, long* result)
{
	...
	if (WM_NCACTIVATE == lpMsg->message)
	{
	...
	else if (WM_NCCALCSIZE == lpMsg->message)
	{
		if (TRUE == wParam)
		{
			if (d->isMaximized())
			{
				QMargins maximizedMargins = d->maximizedMargins();
...
NativeWindowHelperPrivate::NativeWindowHelperPrivate()
	: q_ptr(nullptr), window(nullptr), tester(nullptr), oldWindow(nullptr)
{
}
...
void NativeWindowHelperPrivate::updateWindowStyle()
{
	...
	LONG oldStyle = WS_OVERLAPPEDWINDOW | WS_THICKFRAME | WS_CAPTION |
