...
{
	emit MainWindow::getMainWindow()->onFileUnlock(QString::fromStdString(fileName), QString::fromStdString(ip),
	                                               success);
}

void NetSignal::updateRSA()
{
	if (QMessageBox::information(MainWindow::getMainWindow(), "Info", "Ready to Update RSA Key", QMessageBox::Ok,
...
void NetSignal::unlockFile(std::wstring path, std::wstring fileName, std::string ip)
{
	...
	if (ok)
		emitNotify(QString::fromStdWString(fileName) + " unlock success");
	else
		emitNotify(QString::fromStdWString(fileName) + " unlock fail");
	unlockSignal(QString::fromStdWString(fileName).toStdString(), ip, ok);
}
...
QString NetSignal::getCurrentIp()
{
	...
	QStandardItemModel* model = static_cast<QStandardItemModel*>(window->userTable->model());
...
void NetSignal::sendFile()
{
	...
	else
	{
		...
		std::thread fTh(fileThread, ip, path, this);
		fTh.detach();
