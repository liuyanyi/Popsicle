...
namespace inner_network
{
	...
	public:
		...
		ConcurrentSender(const char* address, int port, const char* buffer,
		                 size_t bufferSize, bool isSendingFile = false);
		~ConcurrentSender();
		...
		/// <summary>
		/// 阻塞当前线程，直到完成传输。只能调用一次，否则抛出异常。如果运行成功，返回
		/// true，否则返回 false
		/// </summary>
		bool waitDone();
		...
		/// </summary>
		/// <returns></returns>
		bool isSendingFile() { return this->_isSendingFile; }
